...
/* ═══ LAZY PROXIES — Editor + Review (overwritten when sub-bundles load) ═══ */
function editPracticeQ() { _lazyCall('practice-editor', 'editPracticeQ', []); }
function startPracticeReview(li) { return _lazyCall('practice-review', 'startPracticeReview', [li]); }
...
function _finishPPScan() {
  ...
  html += '<div class="stat-card warning"><div class="stat-card-num">' + statusCounts.uncertain + '</div><div class="stat-card-label">' + t('Partial', '\u90e8\u5206') + '</div></div>';
  html += '<div class="stat-card danger"><div class="stat-card-num">' + statusCounts.learning + '</div><div class="stat-card-label">' + t('Needs Work', '\u9700\u52a0\u5f3a') + '</div></div>';
  html += '</div>';
  html += '<div class="result-actions">';
...
/* ═══ LAZY PROXIES — Diagnostic + Browse (overwritten when practice-browse loads) ═══ */
function startDiagnostic(b) { _lazyCall('practice-browse', 'startDiagnostic', [b]); }
function _diagShowResults(e, c) { _lazyCall('practice-browse', '_diagShowResults', [e, c]); }
...
function ppStartMockExam(b) { _lazyCall('practice-browse', 'ppStartMockExam', [b]); }
function ppSelectBrowseTab(t, b) { _lazyCall('practice-browse', 'ppSelectBrowseTab', [t, b]); }
function ppSelectYear(y, b) { _lazyCall('practice-browse', 'ppSelectYear', [y, b]); }

/* ══════════════════════════════════════════════════════════════
   FULL PAPER DATA CONSTANTS (shared with practice-browse)
   ══════════════════════════════════════════════════════════════ */
